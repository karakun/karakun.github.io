...
I try to give you a detailed insight into why and how I started to prepare for the exam to the point where I performed the exam.
...
I'm now working with Java since my study days 25 years ago and got first in touch with the language in its 1.0 version.
Mainly developing small applets and for studying computer languages.
From my first day as a consultant back in 2000 I am programming with Java almost every day.
...
So he asked who would be interested to create a working group to do the exam. 
He himself did already a Java certification before.
...
We are quite busy with our daily projects and projects always go first.
It just has to fit with the daily work, which is not always possible.
I bet you know this situation.

As it comes, at that time I was deeply busy with my current main customer project.
It afforded almost all of my time, and there was almost no room to get started.
Fast I was lacking behind the schedule and thought it is not possible to work through the complete book in that time.
But then came easter vacation and "thanks" to the corona crisis and bad weather there was anyway time left then.
Thus I decided to work through the chapters almost each vacation day.
Normally I do not much work during vacation and use the time to relax and spend time with my family.
Working through the book was the opposite of relaxing, and family came also sometime a bit short.
But now I came back on the time track.

At the end of vacation, 1,5 weeks before the exam, I completed chapter 19 of 22, leaving "only" 4 chapters to read to practice exam questions.
But then I was starting a second project, and the main project required focus, too.
So it was left to the evenings and weekends to complete the preparation.

## What I learned from studying the Book

I am quite confident in my Java skills for daily work.
However, goal of the study was to learn especially in areas that I do not use so frequently in my daily work and also to get into details.
When you have a 1300 pages study guide and do not learn many things from it, then you are doing something tremendously well or horribly wrong.

So I went really through the book page by page, reading even the very basics.
Normally I'd tend to just skip parts which I'd think I already know, but I felt it was even worth reading thus and, uhm, study.
It felt right to do so.
After the introduction part there is the first set of Assessment Tests to get a feeling what is awaiting you in the exam questions.
And there I felt that they were harder to answer then expected.
I failed with quite a lot.
From then on I knew that you'll really have to be prepared well to pass.

The book makes it a bit harder for you, because it does not tell how many answers are correct when multiple are.
In the real exam you always know how many answers you have to check.
When doing the review questions after each chapter, I failed with rather many because there was an answer option checked too much or too few.

For all questions I answered wrong I read the explanation in the appendix.
This helps you to understand the topic in detail and also where you have to look for tricks.
...
It describes everything in detail, one topic after the other without overlapping or taking anything for granted.
...
I learned really many details where I do not think about anymore.
The compiler, the IDE, Dr. Google or Mr. Stackoverflow helps me to correct things when I'm doing something wrong, and fixing is often easy.
The book helped to focus on those details and think in front.
...
You'll see that with Java you could write code that is more complicated to understand than Perl scripts written by a drunk gorilla.

Especially tricky were to me nested loops with incrementing different scoped variables with either pre- or post-increment operator and proper usage of `break` or `continue` on labeled loops.
To answer those questions correctly you'll have to track the variable state.
...
It just takes too much time, and one question is as much worth as the other.

For the exam you'll learn all the command line switches and their inconsistencies when they use single or double hyphen.
...
But why you need to memorize which command line options it offers and get tricked by options that might look right?
...
Before the exam I just had to do 7 of those mock exams (there are 22 with ~50 questions each).
...
For the last mock exams I decided to use the option to review after each question, and used this to understand also for correct answers if they were answered correctly because I really hit the nail there.
Sometimes you might be just randomly right, and I avoided false positives.
Of course, this takes time then.

Overall the mock exams do quite well simulate the real exam in their content and extend.
...
You have 2 options to continue: Either get an SMS with a link, or enter the link yourself.
I chose the SMS, but unfortunately I did not receive the message.
The OnVUE software was waiting until you continue on the smartphone and I was blocked.
I needed to restart and the software and then entered the link manually.
...
I got about position 25, and it took between 5 and 10 minutes until I was in front.

Now a proctor opened a chat window and wrote that he could not see my video stream.
This will be surely because I unplugged the external monitor and the webcam during the check-in procedure.
However, since the software still showed me in a video stream, I was surprised that the proctor did not see it, too.
So I needed to restart the software again and go through the check-in procedure again.
...
I came back into the check-in queue, and there it said immediately that I was next.
...
This can be either by more code, or more complex code.
...
I had about 4. Already planned for skipping was nested labeled loops where you need to answer where to insert `break <LABEL>` or `continue <LABEL>` to get a specific output.
...
Could be that I missed some, at the end I made of course some mistakes.
...
Doing this exam is an intense experience and requires well preparation, even for experienced Java developers.
...
Wouldn't I have sacrificed some of the vacation and spare time, I would not have made it through the study material.
I might have passed, but when I would have before studying the whole material, I won't have touched them again.
Not all of our study group made it through the material because they did not have enough time besides their regular work.
They decided to just make the exam later, then of course at regular cost, when they learned completely for it.
...
Skip questions immediately when they'll cost you valuable time reading and understanding them.
