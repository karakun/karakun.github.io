...
This move is planed and coordinated as part of the OpenJDK [project Skara](https://wiki.openjdk.java.net/display/SKARA).
Many believe this to be a major step for the future of the OpenJDK.
By using GitHub and its workflows, developers around the world are able to participate in the development of OpenJDK.
...
Also, the workflows (e.g. the internal progress of code reviews) was quite complex and hard to understand.
Git and GitHub are widely used today, and a majority of developers is familiar with them.
...
If you are interested you can find example of a pull request [here](https://github.com/openjdk/jdk/pull/1465).
...
The good news is, searching and browsing for issues can be done without having an account at the bug database.
If you want to report a bug, on the other hand, you need to have a login i.e. you need ot be an OpenJDK Author.
To make a long story short: getting the OpenJDK Author status is not that easy and definitely nothing you want to do simply to file a bug.
Sadly the front-page of the Jira does not contain any information how somebody who is not an OpenJDK Author can submit a bug.
Only a vague hint to the wiki exists:
...
At the "Oracle Java Bug Database" you can use a simplified search functionality and have the possibility to display the main information of an issue.
The following image shows the above issue in the Oracle Bug Database.

![OpenJDK bugtracker issue](/assets/posts/2021-02-22-openjdk-bug-tracker/oracle-bug.png)

The information which is available "Oracle Java Bug Database" is only a subset of the ones in JBS Jira.
...
Sadly, you almost need to Google such information since there is no explanation of the process within the JBS.
...
It asks us to search the existing database before opening an issue, in order to avoid duplicated reports.
...
The guide is really extensive, so plan some time to read it.
Having all this options and steps explained is very helpful, but wait it the guide is for Java 10.
With a little URL fiddling (simply change the version in the URL) we can get the guid off the version we want to report the bug for.
...
Surly this has some historical or organizational reason and simply is the way it is because it always has been this way.

This is a topic for another time.
We want to stay focused.
Let us have a look on how to submit a bug.
...
Does this mean that We cannot submit a bug if we use a OpenJDK distribution other than the Oracle JDK?
...
Looks like Oracle wants to discourage the creation of new bug reports with all means.
...
Hurray, we are finally on form to create a bug report.
...
Within a "short" period of time (maximum a few days) you will normally receive a mail from Oracle that your issue has been created.
...
That's really handy, especially if you are asked to provide additional information about the issue.

![Mail by Oracle](/assets/posts/2021-02-22-openjdk-bug-tracker/oracle-mail-2.png)

When clicking on the "provide more information about this issue" you get sent to another form of the "Oracle Java Bug Database".
...
Many developers are used work with issue trackers which support markdown to add some minimal formatting to an issue.
...
But you can benefit from this manual step if you want to comment to an issue.
Simply go to the form for reporting an issue, but instead of opening a new ask to add additional information to an existing one.
...
One thing that I totally ignored in this article are the OpenJDK mailing lists.
...
From my point of view this workflow is even complexer than the above one, since you need to understand to which OpenJDK mailing list you need to send such request.

Hint: [there are some of them](https://mail.openjdk.java.net/mailman/listinfo).

It is even harder to find that right mailing list than it is to find the "Oracle Java Bug Database".
And frankly in 2021 most developers do not want to start a mailing list discussion when they found a bug.
...
On the other hand it is clumsy that a not JDK Author cannot create an account and report issues directly in the bug database.

Oracle provides a tedious solution that is half bug tracker and half Oracle support sells point.
...
If it were provided by the OpenJDK maybe we would see several mentions of paid Oracle support or checkboxes telling you that this is not a place to get support.
...
Then place this tutorial prominently on the start page of the JBS, the Github repositories and the Oracle Java Bug Database.

Or maybe [Eclipse Adoptium](https://blog.adoptopenjdk.net/2020/06/adoptopenjdk-to-join-the-eclipse-foundation/) will be able to provide an ad-free und user-friendly frontend to report bugs for the OpenJDK and comment on them.
